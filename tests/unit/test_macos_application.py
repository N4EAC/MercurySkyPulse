...
class MacosApplicationTests(unittest.TestCase):
    ...
    def test_native_program_name_is_set_for_interpreter_launch(self) -> None:
        self.assertTrue(set_macos_program_name("Mercury SkyPulse"))
        getter = ctypes.CDLL(None).getprogname
        getter.argtypes = []
        getter.restype = ctypes.c_char_p
        self.assertEqual(getter().decode("utf-8"), "Mercury SkyPulse")
        self.assertEqual(macos_cocoa_process_name(), "Mercury SkyPulse")
