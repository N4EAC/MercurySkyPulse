...
class MacosBuilderContractTests(unittest.TestCase):
    def test_builder_creates_named_application_bundle(self) -> None:
        ...
        self.assertIn('candidate="${MERCURY_EXECUTABLE:-}"', script)
        self.assertIn("prepare_mercury", script)
        ...
        self.assertNotIn("NSMicrophoneUsageDescription", script)
        self.assertNotIn("validate_voice_package", script)
        self.assertIn("Set :CFBundleName Mercury SkyPulse", script)
        self.assertIn("CFBundleDisplayName string Mercury SkyPulse", script)
